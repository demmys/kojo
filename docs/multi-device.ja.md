...
会話履歴・メモリなどの正本データは省略せず同期します。Claude / Codex / Grok
などのネイティブセッション artifact は、最終更新が新しいものから順に、
agent-sync の 128 MiB (展開後 JSON) 上限へ収まる分だけ転送します。転送
されなかった古い artifact の会話は、次に開いた時に正本の会話履歴を
注入して新しいネイティブセッションとして継続します。正本データだけで
128 MiB を超える場合は chunked agent-sync に切り替わり、履歴を削りません。
