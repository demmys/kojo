...
func TestDeleteGrokThreadRefStrict(t *testing.T) {
	withGrokHome(t)
	home := t.TempDir()
	t.Setenv("HOME", home)
	...
	const agentID = "ag_grok_force_fresh"
	dir := agentDir(agentID)
	writeGrokSessionIDFor(dir, grokKeyA, grokIDA, silentLogger())

	if err := deleteGrokThreadRefStrict(agentID, grokKeyA); err != nil {
		t.Fatalf("delete ref: %v", err)
	}
	if got := readGrokSessionIDFor(dir, grokKeyA); got != "" {
		t.Fatalf("ref survived delete: %q", got)
	}
	if err := deleteGrokThreadRefStrict(agentID, grokKeyA); err != nil {
