...
- A symlink works: `ln -s /path/to/big.mp4 {DATA_DIR}/.kojo/attach/big.mp4` attaches the target's bytes without copying them. Only the link is removed after ingest; the file you linked to stays where it is. The link must point at an existing regular file.
