...
func (m *Manager) Fork(srcID string, opts ForkOptions) (*Agent, error) {
	...
	if err := copyFileIfExists(filepath.Join(srcDir, "anchor.md"), filepath.Join(dstDir, "anchor.md")); err != nil {
		return nil, fmt.Errorf("copy anchor.md: %w", err)
	}
